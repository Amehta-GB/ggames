class Game{
	...
	init() {
		...
		loader.load( `../images/BusinessMan.fbx`, function ( object ) {
            ...
            tLoader.load(`../images/SimplePeople_BusinessMan_Black.png`, function(texture){
				object.traverse( function ( child ) {
					if ( child.isMesh ){
		...
		this.renderer.setSize( 700, window.innerHeight - 200 );
    ...
    createColliders(){
        ...
        const material = new THREE.MeshBasicMaterial({color:0xbbbbbb, wireframe:false});
		...
		socket.on('pos', (d) => {
			console.log('position data coming.....');
			console.log(UUID);
			...
			var parent = this.scene.getObjectByName(id);
			...
			if ( d[1].UUID===UUID){
				...
				if (parent) {
					...
					//box.position.set(d[1].x, d[1].y, d[1].z);
					this.scene.getObjectByName(id).position.x=d[1].x;
					this.scene.getObjectByName(id).position.y=d[1].y;
					this.scene.getObjectByName(id).position.z=d[1].z;
					//this.scene.getObjectByName('id').position.y=d[1].y;
					//this.scene.getObjectByName('id').position.z=d[1].z;
					//this.scene.remove(parent);
							...
							fbxLoader2.load(`${this.assetsPath}FireFighter.fbx`,(object) => {
    ...
    playerControl(forward, turn){
		...
		if (forward==0 && turn==0){
			...
			this.player.move = { forward, turn }; 
				...
				if (this.tempx==this.player.object.position.x){
				}else{
					socket.emit('user-position', { x:this.player.object.position.x , y:this.player.object.position.y , z:this.player.object.position.z,  UUID} )
					this.tempx=this.player.object.position.x;
				}
				//==============================================
